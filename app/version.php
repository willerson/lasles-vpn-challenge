...
if ( ! function_exists( 'fuerza_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function fuerza_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'fuerza_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function fuerza_should_load_wpemerge( $name, $min, $max ) {
		$loaded = fuerza_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'fuerza' );
				?>
...
if ( ! function_exists( 'fuerza_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function fuerza_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = fuerza_get_loaded_wpemerge();
